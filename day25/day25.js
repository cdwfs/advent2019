...
let aoc25 = function() {
    ...
    function processInput() {
        ...
        let result = vm(encodeAsciiInputs(inputText));
        let outputStr = decodeAsciiOutputs(result.value).replace(/\r\n|\n/ug, "<BR>");
        document.querySelector("#textOutput").innerHTML += `<code>> ${inputText}</code><BR>`;
        document.querySelector("#textOutput").innerHTML += outputStr;        
        if (result.done) {
            document.querySelector("#textOutput").innerHTML += `<B>GAME OVER</B>`;
        }
        document.querySelector("#textInput").value = "";
    }

    function loadVm(pgm) {
        vm = aoc.coroutine(intcodeMachine, [pgm,]);
        document.querySelector("#textOutput").innerHTML = "VM loaded!";
    }
    
    function findPassword(pgm) {
        const commands = `\
south
take space heater
east
